...
- [x] `TreasuryJob` and `TreasuryJobTransaction` (D36): the request, each signer's pinned key, the state and reason, where each key is stored, the treasury's address once it exists, and every transaction (raw, hash, nonce, what it was for, what became of it, and what it created); one open job per vault by a partial unique index
- [x] The engine as steps: `advance(job)` does at most one chain action per call, stores every transaction before sending it (`on_prepared`) and settles them with `Relayer.status` (D21). A transaction the node has dropped is broadcast again; one signed by another relayer key is waited on, not settled; a job gives up after two days rather than holding the vault for ever, and an owner can stop one
- [x] Scheduler tick and one `Relayer` per process in `app.extensions`, built from the environment and absent (not fatal) when it is not configured; the tick does nothing while the feature is off
- [x] D37: `signer_preferences`, one answer (`preferred_key`) for the account page, the phone and a link alike, and revoking a phone puts its owner back on their password key
- [x] D38 limits and reserve: checked when a job is asked for — including whether the relayer can afford the whole link (D33) — and again before each transaction, counting what that transaction would cost. Any job that spent gas starts the vault's cooldown, so a failed job is not a way around it
- [x] API: `GET`/`POST /api/v1/vaults/<id>/treasury` (member; owner to create, behind the D25 capability), `POST .../treasury/stop`, `PUT /api/v1/me/signing-choice`; web: a Treasury tab, create and stop, and the key choice on the account page. Both refuse an endpoint that will not answer with a message rather than a 500
- [x] Tests: every state transition, a restart between every pair of steps, a lost send, a dropped transaction, a superseded one, a reverted one, a fee spike, a reserve breach, two owners racing, a vault changed mid-job, a job that gives up, a crash between the treasury's commit and the job's, CSRF on both forms, and the page after a member leaves. Mutation run over the job engine and the choice
- [x] **Adversarial review (2026-09-18) fixes:** no critical. H-1 a mined deployment's address was thrown away, so one failed re-scan would have paid for a second treasury (it is stored from the receipt); H-2 the cooldown counted only finished links and any signer could fail a job by changing their own key choice — 0.0316 ETH burned in the reviewer's run — so the choices are pinned when the job is asked for and any spending counts. M-1 a transaction was settled against whatever relayer key was to hand; M-2 a wrong RPC setting killed a job for good; M-3 nothing could stop or time out a job, and the request did not check it could afford the whole link; M-4 a crash between the two commits wrote the treasury twice and put raw SQL in a member-visible field; M-5 removing a member made the treasury tab a 500; M-6 what a signer chose and what a link registered could differ; M-7 an RPC outage was a 500; M-8 two owners racing got a 500. Lows: a non-numeric key choice, admin-versus-owner disagreement between web and phone, the reserve not counting the spend, the flag unchecked mid-job, the phone able to set a choice where the feature does not exist, dead code, doc drift, a missing `rel=noreferrer`
- **Done when:** a link runs end to end on anvil through the job, with the process restarted at every step, and pays for nothing twice. *(Done 2026-09-18: `tests/test_link_anvil.py::test_a_job_links_a_vault_end_to_end_with_a_restart_at_every_step`.)*

### Phase 6a: Execution signatures (web) · no ETH

- [x] `key_service.sign_messages`: one unlock signs several messages, each verified before any is returned (ADR-0010). `sign_with_key` is now the one-message case of it, so nothing else changed
- [x] `cast_vote`: approving a payment also signs the execution digest → `ExecutionSignature` (its own table, one row per signer, the signed digest and the key pinned beside it), written in the same transaction as the vote; the `proposal_signed` payload gains `execution_signature_sha256`
- [x] Refused before the password is spent, each with the remedy named: a rotated key, a signer the treasury never registered, a seat held by that signer's phone (approve there), a registered key of another algorithm, and a treasury that has been unlinked (D39)
- [x] The device path refuses an approval on a payment that arrives without an execution signature, whatever capability the client declares (D25 is the app's claim; this is the check). Rejecting a payment still needs nothing extra
- [x] The digest is recomputed from the stored payment every time, never cached, and a payment row edited to nonsense is a refusal a person can read rather than a 500
- [x] **Adversarial review (2026-09-18) fixes:** no critical. **H-1** the execution digest was built from the `proposal_actions` row without checking the decision's binding, so a database edit made *before* anyone approved (a new recipient, a larger amount) was signed by every honest approver while the page still showed the original payment; the vote was never at risk (it signs `payload_hash`), the execution signature was, and the contract checks nothing else. The binding is now checked before signing, against the same loaded row the digest is built from; the row is read through the strict D22 parser; and Approve is hidden on a payment whose binding is broken (Reject stays). Found alongside it: an execution signature sent with a **reject** would have been stored as an authorisation to pay, so a signature is now accepted if and only if the vote approves a payment. Lows: a revoked phone's seat sent its owner back to that phone; an orphaned execution signature read as "already voted"; `record` did not tie the key to the signer, re-check the seat after the unlock, or compare the treasury's vault and chain; the digest parser was looser than D22; no stable API code for a missing execution signature; the digest traced twice and its verification not at all. D27 gap: each signature now pins the signer's 124-byte on-chain identity, which Phase 7 submits and a 7b reconfiguration would otherwise rewrite. Weak tests: five "before the password" tests could not tell before from after, forged signatures were never sent through the vote path, and the duplicate-vote branch never ran; each rewritten so it can fail. The rewritten race test found one more: a clash on the execution-signature row could surface at an autoflush (a lazy load of the proposal's votes) outside the block that maps a race to "already voted", as a 500; everything from the first added row to the flush is now inside that block
- **Done when:** tests cover a valid signature, a tampered one, a rotated key, and a reject vote (no execution signature). *(Done 2026-09-18: `tests/test_execution_signatures.py`.)*
...
- [ ] A decision left `open` by two final approvals made at the same moment (each transaction saw M−1) is finalised by the scheduler's own tally rather than waiting for ever. Pre-existing, noticed in the 6a review; matters now because an approved payment that is never marked approved is never executed
- [ ] Submit each signature with its pinned `identity_hex`, and refuse to submit one whose identity the treasury no longer holds
...
- [ ] ⚑ **Decide before the first treasury is deployed (Phase 9):** `executionDigest` does not cover `configNonce`, so a reconfiguration that *lowers* the threshold makes the partial approvals of any still-open payment executable on chain by whoever holds them — and Phase 9 publishes execution signatures in the decision record. Either bind the digest to `configNonce` (a contract, digest and fixture change, free while no treasury exists and impossible after; a reconfiguration then voids open payment approvals, which is arguably the right meaning) or refuse to lower a threshold while payment decisions are open. Found in the 6a review
...
| 2026-09-17 | 5 | **Re-scoped for production** (owner: "i want it to be like its production not a one time demo"). D35 production on Sepolia, mainnet only after an audit; D36 owners create a treasury in the app and the server links it as a resumable job; D37 each signer chooses the key the treasury registers for them; D38 the operator's relayer pays, with a reserve, per-vault limits and warnings; D39 a treasury follows its vault by `reconfigure` instead of relinking; D40 a treasury page on web and phone; D41 a runbook. Phase 5 split into 5a (the engine, landed) and 5b (linking in the app); Phase 7b added; Q2 and Q3 answered by D37 and D36 | `3288a27` |
| 2026-09-18 | 5b | **Linking in the app landed.** An owner creates a treasury from the web or the phone; the scheduler advances a durable job one chain action at a time, storing each transaction before it is sent; each signer chooses which of their keys it registers; the relayer's reserve and a per-vault cooldown bound what can be spent. Adversarial review: no critical, 2 high (a mined deployment's address thrown away, so a missed re-scan paid for a second treasury; and the cooldown counting only finished links, which let any signer burn the relayer's ETH by changing their own key choice — 0.0316 ETH in the reviewer's run), 8 medium, 9 low, all fixed with a test each. Mutation run over the new code: 24 planted bugs, 6 of them unnoticed at first — the algorithm and the owner of a chosen phone key went unchecked, the *first* transaction for a step was read instead of the last (which would pay 8.7M gas for a registration already in flight), the record named the first signer rather than whoever asked, and one signed transaction could be stored twice. A test each; the sixth was an exception name the class above it already covered, so the list says what it means instead | `63d0f1b` |
| 2026-09-18 | 6a | **An approval on a payment now authorises the chain.** One password produces two signatures: the vote, and one over the digest `QVaultTreasury` itself checks before it moves ETH. The contract counts keys, not people, so an approval that the treasury could not accept — a rotated key, a seat held by the signer's phone, an unlinked treasury — is refused before the password is spent and says what fixes it. The device path refuses a payment approval that arrives without its execution signature, so a client cannot mark a payment approved that nothing could execute. Adversarial review: no critical, **1 high** — the digest was built from the payment row without checking the decision's binding, so a database edit made before anyone approved (reproduced through the real web route: page shows the original recipient, the stored signature pays another) would have been signed by every honest approver; the binding is now checked before signing and Approve is hidden on a broken payment. Found beside it: a rejection could have carried an execution signature, stored as an authorisation to pay; now present if and only if the vote approves a payment. The new race test then caught one more: a clash on the execution-signature row could surface at an autoflush outside the block that maps a race to "already voted", as a 500. 8 lows fixed, the signer's on-chain identity pinned on each signature, and five tests that could not tell "before the password" from "after" rewritten | *(this commit)* |
