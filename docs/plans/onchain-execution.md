...
- [ ] `scripts/record_deployment.py`: read forge's `broadcast/.../run-latest.json`, confirm receipts and `eth_getCode` at each address, then **merge** into `chain/deployments/sepolia.json` (addresses, transaction hashes, commit), never overwrite. *Written and tested against the fake node (`qvault/chain/deployments.py`, 26 tests). It also requires the verifier's on-chain code to equal this commit's build byte for byte, with the one immutable holding the helper's address.*
- [ ] Live check: `eth_call` accepts a quantcrypt signature and rejects a tampered one. *`scripts/check_verifier_live.py` uses `eth_call` state overrides to place the key halves at unused addresses, so it needs no `setKey` (~8.8M gas) and no ETH. Dry-run on 2026-09-17 with the helper and this commit's verifier also supplied by override: genuine signature `0x024ad318`, tampered and wrong-digest `0xffffffff`, so Alchemy supports overrides and the encoding is right.*
...
| 2026-09-17 | 2 | Phase 2 landed: RPC client, relayer, fake node; 146 tests | *(this commit)* |
