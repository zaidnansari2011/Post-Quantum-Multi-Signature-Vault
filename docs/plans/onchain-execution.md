...
- [ ] `qvault/chain/rpc.py`: minimal JSON-RPC client; `qvault/chain/relayer.py`: build, sign and send EIP-1559 transactions, wait for receipts
- [ ] pytest: a fake RPC covers the transaction lifecycle (**no network in tests**)
- **Done when:** the full suite is green.

### Phase 3: Shared contracts on Sepolia · ~0.01 ETH

- [ ] Deploy the F1600 helper and `ZKNOX_dilithium65`; verify the source on Etherscan
- [ ] `scripts/record_deployment.py`: read forge's `broadcast/.../run-latest.json`, confirm receipts and `eth_getCode` at each address, then **merge** into `chain/deployments/sepolia.json` (addresses, transaction hashes, commit), never overwrite
- [ ] Live check: `eth_call` accepts a quantcrypt signature and rejects a tampered one
- **Done when:** both contracts are verified on Etherscan and the live check passes.
...
- [ ] Idempotent across crashes: a submitted transaction is reconciled from its hash or from the `Executed` event, never re-sent blindly; `AlreadyExecuted` (someone else submitted first) counts as success (D20)
...
| 2026-09-17 | 1 | Phase 1 landed: treasury contract, Python digests and key expansion, cross-checked fixtures, deploy script, CI `contracts` job. Foundry 65/65, pytest 1,057 passed | *(hash recorded with Phase 2)* |
