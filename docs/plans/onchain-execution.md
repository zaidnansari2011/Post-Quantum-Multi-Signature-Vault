...
| D33 | **Refuse before spending.** A dry run is the default; `--broadcast` is required to spend. The dry run prints the vault, each signer with the key and custody to be registered, keys already on chain, the estimated gas and cost of each step at the current fee, and the balance each step needs up front. It refuses clearly when a signer has no eligible key (SLH-DSA, ML-DSA-87, no active key, or not exactly one active phone key under `--device`), when N > 10 or M > 8, when the vault is already linked, when the database lacks a column the link uses, when the base fee is above the D21 policy, or when the balance cannot cover the whole sequence. | The relayer holds ~0.039 ETH for a ~0.03 ETH link. A link that stops halfway loses nothing (D30), but the owner should see the cost and the keys before approving a broadcast. | Broadcasting by default |
| D34 | **A payment requires the vault's signers to be exactly the treasury's.** Creating a payment compares the vault's signer user ids with the treasury's `treasury_signers`, not only their number. Changing a linked vault's members or threshold stays allowed; payments are refused until a relink (a reconfiguration flow is out of scope). | Swapping one member keeps N the same, while the newcomer has no key on the treasury and the person who left still has one. | Blocking membership changes on linked vaults |
...
- [ ] `qvault/chain/artifacts/QVaultTreasury.json` and `scripts/export_treasury_artifact.py` (D32); the CI `contracts` job checks the committed file against a fresh build
- [ ] Model `TreasurySigner` (D29), registered; `treasury_linked` added to the audit sentences; `AUTO_CREATE_DB` readable from the environment (D28)
- [ ] `qvault/chain/key_storage.py`: find a key's canonical storage among the verifier's contracts; find a confirmed `setKey`'s pointers in the verifier's nonce range; read-back comparison (D30)
- [ ] `qvault/chain/treasury_check.py`: the D31 check over RPC, reporting every mismatch rather than the first
- [ ] `qvault/services/treasury_service.py`: `plan_link` (eligibility, D33 refusals, costs), `link` (D30 order: reuse or `setKey` each key, reuse or deploy the treasury, wait for finality, D31 check, then one database transaction with `treasury_linked`) and `check`. The relayer and RPC client are passed in
- [ ] `scripts/link_treasury.py --vault <id> --by <username> [--device <username>]... [--broadcast | --check]`: dry run by default (D33); opens the app without startup writes (D28); prints the Etherscan verification command; records the treasury in `chain/deployments/sepolia.json` (vault id, threshold, and per signer the user id, custody, public-key SHA-256 and on-chain key id; deployment transaction and block; status). **No names in the record**, which is public
- [ ] `_payment_action` compares signer sets, not counts (D34)
- [ ] **Reseed guard (D16):** `seed_demo.py --reset` (before `drop_all`) and `migrate_to_postgres.py --force` refuse when this database has a linked treasury, or holds a key registered on a treasury the record lists as linked (a restored backup or a copy). `--unlink-treasury` prints the treasury, its balance (locked for good), the approvers who lose access and the cost of relinking, then marks the treasury unlinked in this database and in the record. Tests cover both refusals and the override
- [ ] Tests: the fake node gains contracts that create contracts only when a transaction is included (`setKey`) and a fake treasury answering the views; every D31 mismatch; a crash and re-run at each step pays for nothing twice; a dry run writes nothing (database and record byte-identical). **Local anvil end to end:** real `setKey` and deployment from the committed artefact, then the D31 check; the measured gas is recorded here
