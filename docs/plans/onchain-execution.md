...
- [ ] `qvault/chain/action.py` (pure): `EthTransfer` built from a recipient and an amount under the D23 policy, strict `parse` of the D22 shape (refuses every malformed field, extra key or unsafe number), `canonical()` dict, exact wei formatting and the D24 text, and `execution_digest(payload_hash)` equal to `digest.execution_digest`
- [ ] Models (D27): `Treasury` (vault, chain, address, verifier, threshold, deployment tx and block, status, linked/unlinked times) and `ProposalAction` (proposal, treasury, every signed field); registered in `qvault/models/__init__.py`
- [ ] `proposal_signing_bytes(..., action=None)` adds `action` only when given; `signing_bytes_for` includes the stored action; the frozen vectors still pass; a frozen payment vector is added
- [ ] `create_proposal(..., payment=...)` keyword-only: requires `ONCHAIN_EXECUTION_ENABLED`, a linked treasury whose threshold matches the vault's, and the deadline policy; generates the text; `proposal_created` ledger entry gains `has_action` only for payments
- [ ] Binding: tampering with any stored action field is caught by `verify_proposal_binding`, like any other signed field
- [ ] API: `POST /vaults/<id>/proposals` accepts `payment {to, value_wei}`; `GET /proposals/<uuid>` adds `signing_inputs.action` behind the D25 capability, otherwise 426 `upgrade_required`, and voting is gated the same way; proposal summaries say whether a decision is a payment
- [ ] Exports and the offline verifier: bundles with an action are `qvault.decision/2` (D26); the Python verifier and CLI accept `/1` and `/2` and show the payment
- [ ] Browser verifier: `proposalBytes` includes `action`, accepts `/2`, shows the payment and states what the hash covers; rebuilt; Playwright agreement tests on payment bundles (local)
- [ ] Phone: `signing.ts` and the strict schema accept an optional `action`; `test_mobile_canonical.py` gains payment cases, so Python and the app's TypeScript agree byte for byte. The capability header is not sent yet (Phase 6b)
- [ ] `ONCHAIN_EXECUTION_ENABLED` (off by default, also in tests unless a test turns it on); verification and display of an existing payment decision never depend on it
