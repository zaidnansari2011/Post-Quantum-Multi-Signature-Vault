...
**keys** — every signing **and** KEM keypair, active or retired. `id` PK · `owner_id` FK→users · **`role`** ('sig'|'kem') · `alg_id` · **`backend`** ('liboqs'|'pqcrypto') · `public_key` BLOB · `secret_key_wrapped` BLOB · `secret_key_nonce` BLOB · **`wrap_domain`** ('password'|'master') · `status` ('active'|'retired') · **`can_sign`** BOOL · **`can_verify`** BOOL · `version` · **`rotate_after`** (max-age policy, nullable) · `created_at` · `retired_at`. *At most one active key per (owner, role); enforced in service / partial unique index. Retire-but-retain keeps old public keys.*
